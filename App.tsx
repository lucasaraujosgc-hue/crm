...
export default function App() {
    ...
    const interval = setInterval(async () => {
      try {
        ...
        if (data.status === 'connected' && activeTab === 'chat') {
            ...
            // Apenas atualiza se houve mudança para evitar re-render desnecessário
  ...
  // Poll Active Chat Messages (Fix Scroll Issue)
  useEffect(() => {
    ...
    const fetchMessages = async () => {
        try {
            ...
            setMessages(prev => {
                // Só atualiza se o array for diferente
  ...
  // Scroll to bottom ONLY when messages length increases (new message arrives)
  useEffect(() => {
    if (messages.length > 0) {
        // Verifica se a última mensagem é nova
        const lastMessage = messages[messages.length - 1];
        // Pequena otimização: rola só se for nova
